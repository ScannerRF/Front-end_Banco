...
#======================criando a janela======================
telaDir_EntradasAluno = ctk.CTk(fg_color='#f1f1f1')

#======================configurando a janela principal======================
telaDir_EntradasAluno.title('Diretor - Cadastrar Aluno')
telaDir_EntradasAluno.geometry('1260x960')
telaDir_EntradasAluno.resizable(False, False)
telaDir_EntradasAluno._set_appearance_mode('dark')

#======================funcao======================
#funcao para iniciar a janela centralizada
def centralizarTela(telaDir_EntradasAluno):
    telaDir_EntradasAluno.update_idletasks()
    width = telaDir_EntradasAluno.winfo_width()
    height = telaDir_EntradasAluno.winfo_height()
    screen_width = telaDir_EntradasAluno.winfo_screenwidth()
    screen_height = telaDir_EntradasAluno.winfo_screenheight()
    x = (screen_width // 2) - (width // 2)
    y = (screen_height // 2) - (height // 2)
    telaDir_EntradasAluno.geometry('{}x{}+{}+{}'.format(width, height, x, y))

#funcao para centralizar a janela
centralizarTela(telaDir_EntradasAluno)
...
imgPesquisar = ctk.CTkImage(light_image=Image.open('imagens/imgPesquisar.png'), dark_image=Image.open(
    'imagens/imgPesquisar.png'), size=(26, 26))

#label com imagem do plano de fundo, primeiro item a executar para ser a ultima camada
lblPlanoDFundo = ctk.CTkLabel(telaDir_EntradasAluno, image=imgPlanoDFundo, text='')
...
frmFundoMenu = ctk.CTkFrame(telaDir_EntradasAluno, width=1260, height=120, corner_radius=0,
    fg_color='white')
...
frmFundoMain = ctk.CTkFrame(telaDir_EntradasAluno, width=1060, height=740, corner_radius=0,
    fg_color='white')
...
etyPesquisar = ctk.CTkEntry(frmFundoMain, width=600, height=30, placeholder_text='Pesquisar entrada de um aluno específico por RM',
    font=('arial bold', 12), border_color='green', fg_color='#069E6E', text_color='white',placeholder_text_color='white',
    corner_radius=0)
etyPesquisar.place(x=258, y=160)

#label com imagem icone pesquisar
lblImgPesquisar= ctk.CTkLabel(frmFundoMain, text='', bg_color='#047b55',width=30, height=30,
    text_color='#1C1C1C', image=imgPesquisar, corner_radius=20)
lblImgPesquisar.place(x=202, y=160)
...
frmFundoMenuLateral = tk.Frame(telaDir_EntradasAluno, width=100, bg="#069E6E")
...
#rodando a janela
telaDir_EntradasAluno.mainloop()
